...
- **수업(입장 코드) 세션은 종료 시간이 되면 무조건 끊긴다** (`lib/class-session-policy.js`). 유예도 자동 연장도 없다 — 넣으면 선생님이 정한 종료 시각이 의미를 잃는다. 수업을 더 하려면 새 입장 코드를 발급한다. 규칙은 순수 함수로 분리해 뒀으니 `test/class-session-policy.test.js`로 검증한다
- **이건 게스트(입장 코드) 학생에게만 해당한다.** 로그인 계정(강사·관리자)은 위의 슬라이딩 세션을 따르므로, 수업 코드 없이 그냥 자료를 띄워 놓은 화면은 시간 제한 없이 유지된다
...
- **`refreshMe()`는 상태가 실제로 바뀌었을 때만 `navigate()`를 부른다.** `navigate()`는 첫 줄에서 `presentExit()`으로 발표 오버레이를 걷어내므로, 무조건 부르면 **전체화면 발표가 5분마다 저절로 종료되고 입력 중이던 폼도 날아간다**
