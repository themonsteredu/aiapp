...
test('랜딩은 핵심 접근성 랜드마크와 모션 감축 설정을 갖는다', () => {
  ...
  assert.match(html, /id="silk-ribbon-field"/);
...
test('실크 리본은 장식용 경량 캔버스로 동작한다', () => {
  const canvas = html.match(/<canvas\b[^>]*id="silk-ribbon-field"[^>]*>/);
