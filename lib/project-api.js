...
function publicProject(row) {
  return {
    ...
    startsOn: dateInTimezone(row.starts_on),
    endsOn: dateInTimezone(row.ends_on),
    mission: row.mission,
...
function publicAppUrl(req, slug) {
  ...
  // 플랫폼은 /class 이하에 붙어 있다 (루트는 AI연구소 홈페이지)
  return `${proto}://${host}/class#/p/${encodeURIComponent(slug)}`;
}
...
function registerProjectRoutes(deps) {
  ...
  route('POST', /^\/api\/project\/current\/publish$/, 'student', async (req, res, ctx) => {
    ...
    json(res, 200, {
      ok: true,
      slug,
      url: `/class#/p/${encodeURIComponent(slug)}`,
