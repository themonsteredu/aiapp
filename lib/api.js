...
const { q, one, withTransaction, ready, log, getSettings, setSetting, clientIp, TS, KST_TODAY, DEFAULT_AGREEMENT } = require('./db');
...
const { dateInTimezone } = require('./timezone');
const { generateSecret, verifyTotp } = require('./totp');
const storage = require('./storage');
const { registerProjectRoutes } = require('./project-api');
const { registerCareerLogRoutes } = require('./career-log');
const { createSchoolAccounts, isSchoolUsername } = require('./school-accounts');
const { createSchoolRegistry } = require('./school-registry');
const { registerSchoolRegistryRoutes } = require('./school-registry-api');

// 모아허브가 발급한 학교 학생 계정 로그인 (lib/school-accounts.js). 진로기록 학생 번호를 모아허브와 공유한다.
const school = createSchoolAccounts({ q, one, withTransaction });
...
function publicUser(u) {
  return {
    ...
    schoolAccount: !!u.school_account_id,
    agreedVersion: u.agreed_version || 0,
    projectTeamId: u.project_team_id || null,
    anonymousNo: u.anonymous_no || '',
...
async function needsAgreement(user, settings) {
  ...
  // 진로업체 담당자도 제외한다 — 이 계약서는 모아랩 강사용이고, 동의 화면 자체가 강사 전용 API라 걸어 두면 로그인만 되고 아무것도 못 한다.
  if (user.role === 'student' || user.role === 'partner' || user.role === 'superadmin' || user.guest_session_id) return false;
...
function clientSettings(settings, user) {
  ...
  // 허용 IP 목록은 운영 정보다. 학생과 외부 진로업체 담당자에게는 내려보내지 않는다.
  if (!user || user.role === 'student' || user.role === 'partner') delete out.ip_allowlist;
...
function canManage(actor, targetRole) {
  ...
  if (actor.role === 'admin') return ['instructor', 'partner', 'student'].includes(targetRole);
  // 강사는 학생만 관리한다 — 진로업체 담당자 계정은 관리자 전용이다.
...
function canAssignRole(actor, role) {
  ...
  // 진로업체 담당자 계정은 관리자 이상만 만든다 (외부 인력에게 학생 기록을 여는 계정이라 강사가 못 만든다)
  if (role === 'partner') return roleLevel(actor.role) >= roleLevel('admin');
...
registerCareerLogRoutes({
  route, q, one, withTransaction, json, roleLevel, guestDeckAccess,
  deckVisibleToStudent, checkAccess, todayInTimezone, cookieSecure: COOKIE_SECURE,
  schoolStudentId: (user) => school.careerStudentId(user),
  // 활동 사진 열람. schoolRegistry 는 아래에서 만들지만 호출은 요청 때 일어나므로 순서 문제는 없다.
  photoForStudent: (studentUuid, photoId) => schoolRegistry.photoForStudent(studentUuid, photoId),
  photoForStaff: (user, photoId) => schoolRegistry.photo(user, photoId),
});

// 학교·기관 학생 계정 발급 (중앙 moakit_accounts, 모아허브와 같은 계정). lib/school-registry.js
const schoolRegistry = createSchoolRegistry({ withTransaction, roleLevel });
registerSchoolRegistryRoutes({ route, one, json, log, registry: schoolRegistry });
// 학생 기록 열람 메뉴 표시용. 관리자는 항상, 강사는 record_access 권한을 받았을 때만.
async function canViewRecords(user) {
  try { return await schoolRegistry.canViewRecords(user); } catch { return false; }
}
...
// 진로업체 담당자(partner)가 부를 수 있는 API. 이 목록에 없으면 역할 서열과 무관하게 막는다.
// 역할 서열만으로는 minRole 'student' 경로(수업 자료·시간표·웹앱 등)가 열려 버리기 때문에 화이트리스트로 한 번 더 좁힌다.
const PARTNER_ALLOW = [
  /^\/api\/login$/, /^\/api\/logout$/, /^\/api\/me$/, /^\/api\/password$/, /^\/api\/settings$/,
  /^\/api\/school-accounts\/record-schools$/,
  /^\/api\/school-accounts\/schools\/[0-9a-f-]{36}\/record-students$/i,
  /^\/api\/school-accounts\/schools\/[0-9a-f-]{36}\/students\/[0-9a-f-]{36}\/records(\/|$)/i,
  /^\/api\/career-photos\/[0-9a-f-]{36}$/i,
];
...
route('POST', /^\/api\/login$/, null, async (req, res, ctx) => {
  ...
  let user;
  if (isSchoolUsername(loginName)) {
    // 모아허브 학교 학생 계정: 중앙 계정 테이블로 검증하고, 연결된 모아랩 학생 계정으로 들어온다
    let linked;
    try { linked = await school.login(loginName, String(password), clientIp(req)); }
    catch (e) {
      if (!e.status) throw e;
      await log(null, 'login_failed', `school username=${loginName.slice(0, 50)}`, req);
      return json(res, e.status, { error: e.message });
    }
    user = await one(`${USER_SELECT} WHERE id = $1`, [linked.id]);
  } else {
    user = await one(`${USER_SELECT} WHERE username = $1`, [loginName]);
    // 학교 계정에 연결된 행은 모아랩 비밀번호가 없다 — 학교 계정 아이디로만 들어온다
    if (user?.school_account_id) user = null;
    if (!user || !verifyPassword(String(password), user.password_hash)) {
      await log(user || null, 'login_failed', `username=${loginName.slice(0, 50)}`, req);
  ...
  res.end(JSON.stringify({
    user: { ...publicUser(user), canViewRecords: await canViewRecords(user) }, access: await checkAccess(),
...
route('GET', /^\/api\/me$/, 'student', async (req, res, ctx) => {
  ...
  json(res, 200, {
    user: { ...publicUser(ctx.user), canViewRecords: await canViewRecords(ctx.user) }, access: await checkAccess(), settings,
...
route('GET', /^\/api\/agreement\/admin$/, 'superadmin', async (req, res) => {
  ...
  json(res, 200, {
    ...
    default: DEFAULT_AGREEMENT, defaultIsNewer: (s.agreement_text || '').trim() !== DEFAULT_AGREEMENT.trim(),
    records, pending,
  });
});
...
// ================= 수업 입장 코드 (1회성 학생) =================
// 일반 수업은 코드 + 표시 이름, 프로젝트 수업은 코드 + 팀코드 + 익명번호로 입장한다.
route('POST', /^\/api\/join$/, null, async (req, res, ctx) => {
  const { code, name, team_code, anonymous_no } = ctx.body || {};
  let cleanName = String(name || '').trim().slice(0, 30);
  ...
  if (!cs || !cs.active || new Date(cs.expires_at) <= new Date()) {
    await log(null, 'join_failed', `code=${String(code).slice(0, 10)}`, req);
    return json(res, 404, { error: '유효하지 않거나 종료된 입장 코드입니다.' });
  }
  let projectTeamId = null;
  let anonymousNo = '';
  if (cs.project_id) {
    const project = await one('SELECT status, starts_on, ends_on FROM projects WHERE id = $1', [cs.project_id]);
    const today = todayInTimezone();
    if (!project || project.status === 'archived') return badRequest(res, '운영이 종료된 프로젝트입니다.');
    const startsOn = dateInTimezone(project.starts_on);
    const endsOn = dateInTimezone(project.ends_on);
    if (startsOn && startsOn > today) {
      return badRequest(res, '아직 프로젝트 운영기간이 시작되지 않았습니다.');
    }
    if (endsOn && endsOn < today) {
      return badRequest(res, '프로젝트 운영기간이 종료되었습니다.');
    }
    const teamCode = String(team_code || '').trim();
    anonymousNo = String(anonymous_no || '').trim().slice(0, 12);
    if (!/^\d{4}$/.test(teamCode)) return badRequest(res, '팀코드는 4자리 숫자입니다.');
    if (!/^[A-Za-z0-9가-힣_-]{1,12}$/.test(anonymousNo)) {
      return badRequest(res, '익명번호는 1~12자의 문자·숫자로 입력하세요.');
    }
    const team = await one(
      'SELECT * FROM project_teams WHERE project_id = $1 AND team_code = $2',
      [cs.project_id, teamCode]
    );
    if (!team) return badRequest(res, '프로젝트의 팀코드가 올바르지 않습니다.');
    projectTeamId = team.id;
    cleanName = `${team.team_name} ${anonymousNo}`;
  } else if (!cleanName) {
    return badRequest(res, '표시 이름을 입력하세요.');
  }

  // 프로젝트 재접속은 같은 익명번호의 임시 계정을 재사용해 팀 작업을 복구한다.
  let u = projectTeamId ? await one(
    `SELECT *, ${TS('created_at')} AS created_at FROM users
     WHERE guest_session_id = $1 AND project_team_id = $2 AND anonymous_no = $3
     ORDER BY id DESC LIMIT 1`,
    [cs.id, projectTeamId, anonymousNo]
  ) : null;
  if (!u) {
    const username = `guest${cs.id}-${crypto.randomBytes(3).toString('hex')}`;
    u = await one(
      `INSERT INTO users
       (username, password_hash, name, role, class_name, created_by, guest_session_id, project_team_id, anonymous_no)
       VALUES ($1, $2, $3, 'student', $4, $5, $6, $7, $8)
       RETURNING *, ${TS('created_at')} AS created_at`,
      [
        username,
  ...
  await log(u, 'guest_joined', `code=${cs.code} 수업=${cs.title}${projectTeamId ? ` team=${projectTeamId}` : ''}`, req);
...
route('PATCH', /^\/api\/class-sessions\/(\d+)$/, 'instructor', async (req, res, ctx) => {
  ...
  if (!cs) return notFound(res);
  if (!canManageSession(ctx.user, cs)) return forbidden(res);
  if (cs.project_id) return badRequest(res, 'AI 프로젝트 수업은 AI 프로젝트 메뉴에서 관리하세요.');
...
route('POST', /^\/api\/password$/, 'student', async (req, res, ctx) => {
  ...
  if (ctx.user.school_account_id) {
    // 학교 학생 계정: 중앙 계정의 비밀번호를 바꾼다 (모아허브·모아랩 공통)
    if (!validPassword(next)) return badRequest(res, '새 비밀번호는 8자 이상이어야 합니다.');
    try { await school.changePassword(ctx.user, String(current || ''), String(next)); }
    catch (e) {
      if (!e.status) throw e;
      return json(res, e.status, { error: e.message });
    }
    await log(ctx.user, 'password_changed', '학교 학생 계정 (모아허브 공통)');
    return json(res, 200, { ok: true });
  }
...
route('POST', /^\/api\/users$/, 'instructor', async (req, res, ctx) => {
  ...
  if (isSchoolUsername(String(username))) {
    return badRequest(res, 'm으로 시작하는 21자 아이디는 모아허브 학교 학생 계정 형식이라 모아랩에서 만들 수 없습니다.');
  }
...
route('PATCH', /^\/api\/users\/(\d+)$/, 'instructor', async (req, res, ctx) => {
  ...
  if (target.school_account_id && (name !== undefined || role !== undefined || class_name !== undefined)) {
    return forbidden(res, '학교 학생 계정의 이름·반·역할은 모아허브 학교 명단에서 관리합니다.');
  }
...
route('POST', /^\/api\/users\/(\d+)\/reset-password$/, 'instructor', async (req, res, ctx) => {
  ...
  if (target.school_account_id) return forbidden(res, '학교 학생 계정 비밀번호는 모아허브의 학교 관리 화면에서 초기화합니다.');
  if (!validPassword(password)) return badRequest(res, '비밀번호는 8자 이상이어야 합니다.');
...
async function handleApi(req, res, pathname, body) {
  ...
  for (const r of routes) {
    ...
    if (r.minRole !== null) {
      ...
      if (user.role === 'partner' && !PARTNER_ALLOW.some((re) => re.test(pathname))) {
        return forbidden(res, '진로업체 담당자 계정은 학생 기록 화면만 이용할 수 있습니다.');
      }
...
module.exports = { handleApi, PARTNER_ALLOW };
