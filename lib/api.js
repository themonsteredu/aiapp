...
route('POST', /^\/api\/login$/, null, async (req, res, ctx) => {
  ...
  res.end(JSON.stringify({ user: publicUser(user), access: await checkAccess(), settings: clientSettings(settings, user) }));
});
...
// 수업 세션 관리 권한: 강사는 자기 세션만, 관리자 이상은 전체
function canManageSession(user, cs) {
  return roleLevel(user.role) >= roleLevel('admin') || cs.created_by === user.id;
}
...
route('GET', /^\/api\/class-sessions$/, 'instructor', async (req, res, ctx) => {
  ...
  const rows = await q(
    `SELECT cs.*, ${TS('cs.created_at')} AS created_at, ${TS('cs.expires_at')} AS expires_text,
     ...
     FROM class_sessions cs LEFT JOIN decks d ON d.id = cs.deck_id
     ${roleLevel(ctx.user.role) >= roleLevel('admin') ? '' : 'WHERE cs.created_by = $1'}
...
route('POST', /^\/api\/class-sessions$/, 'instructor', async (req, res, ctx) => {
  const { title, deck_id, duration_minutes } = ctx.body || {};
  ...
  for (let i = 0; i < 10 && !cs; i++) {
    ...
    try {
      cs = await one(
        `INSERT INTO class_sessions (code, title, deck_id, created_by, expires_at)
         VALUES ($1, $2, $3, $4, now() + ($5 || ' minutes')::interval) RETURNING *`,
        [code, String(title).trim().slice(0, 100), deckId, ctx.user.id, minutes]
      );
    } catch (e) {
...
route('PATCH', /^\/api\/class-sessions\/(\d+)\/live$/, 'instructor', async (req, res, ctx) => {
  ...
  if (cs.deck_id && deckId !== cs.deck_id) {
...
route('GET', /^\/api\/decks$/, 'student', async (req, res, ctx) => {
  // 게스트: 수업에 지정된 웹앱만 (지정이 없으면 공개 중인 전체)
  if (ctx.guestSession) {
...
route('POST', /^\/api\/decks$/, 'instructor', async (req, res, ctx) => {
  const { title, description, subject, kind, external_url } = ctx.body || {};
  if (!title || !String(title).trim()) return badRequest(res, '제목을 입력하세요.');
  const deckKind = kind === 'link' ? 'link' : 'slides';
  let url = '';
  if (deckKind === 'link') {
  ...
  const r = await one(
    'INSERT INTO decks (title, description, subject, created_by, kind, external_url) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
...
route('GET', /^\/api\/decks\/(\d+)$/, 'student', async (req, res, ctx) => {
  ...
  if (ctx.guestSession) {
    const cs = ctx.guestSession;
    const allowed = cs.deck_id
      ? cs.deck_id === deck.id
      : deckStatus(deck, todayInTimezone()) === 'live';
    if (!allowed) return forbidden(res, '이 수업에서 열람할 수 없는 자료입니다.');
...
route('GET', /^\/api\/gate-token\/(\d+)$/, 'student', async (req, res, ctx) => {
  ...
  if (ctx.guestSession) {
    const cs = ctx.guestSession;
    const allowed = cs.deck_id ? cs.deck_id === deck.id : deckStatus(deck, todayInTimezone()) === 'live';
    if (!allowed) return forbidden(res, '이 수업에서 이용할 수 없는 웹앱입니다.');
