...
const { createSession, destroySession, getSessionUser, cleanupSessions, roleLevel, SESSION_MAX_AGE } = require('./auth');
...
const { classSessionState } = require('./class-session-policy');
...
// SameSite=Strict 는 외부 링크(카카오톡·구글 클래스룸·QR 스캐너 등)를 타고 들어올 때 쿠키를 아예 안 보낸다.
// 그래서 로그인이 살아 있는데도 첫 화면에서 튕겨 로그인 창이 뜬다.
// Lax 는 다른 사이트에서 넘어오는 GET 이동에만 쿠키를 허용하고 크로스사이트 POST 는 계속 막으므로 CSRF 방어는 유지된다.
function sessionCookie(token, maxAge = SESSION_MAX_AGE) {
  return `session=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=${maxAge}${COOKIE_SECURE}`;
}
...
route('POST', /^\/api\/logout$/, 'student', async (req, res, ctx) => {
  ...
  res.writeHead(200, {
    'Content-Type': 'application/json; charset=utf-8',
    'Set-Cookie': `session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0${COOKIE_SECURE}`,
  });
...
route('POST', /^\/api\/join$/, null, async (req, res, ctx) => {
  ...
  // 만료 직후 유예 시간 안이면 재입장을 허용한다 (접속이 끊겨 다시 들어오는 학생 구제).
  // 실제 연장은 아래 resolveGuestClassSession 이 판단한다.
  if (!cs || (await resolveGuestClassSession(cs)) !== null) {
...
// 게스트가 붙어 있는 수업 세션이 아직 유효한지 판정하고, 필요하면 만료 시각을 밀어준다.
// 유효하면 null, 끝났으면 학생에게 보여줄 안내 문구를 반환한다.
// guestSession 객체는 연장된 expires_at 으로 갱신되어 이후 응답에 반영된다.
async function resolveGuestClassSession(guestSession) {
  const verdict = classSessionState(guestSession, Date.now());
  if (verdict.action === 'end') return verdict.message;
  if (verdict.action === 'ok') return null;

  const until = verdict.until.toISOString();
  const updated = await one(
    'UPDATE class_sessions SET expires_at = $2 WHERE id = $1 RETURNING *',
    [guestSession.id, until]
  );
  if (updated) guestSession.expires_at = updated.expires_at;
  // 학생들의 접속 세션 만료도 같이 밀어준다 (수업 세션에 묶여 발급됐으므로)
  await q(
    `UPDATE sessions SET expires_at = $2
     WHERE user_id IN (SELECT id FROM users WHERE guest_session_id = $1)`,
    [guestSession.id, until]
  );
  return null;
}
...
async function handleApi(req, res, pathname, body) {
  ...
  for (const r of routes) {
    ...
    if (r.minRole !== null) {
      ...
      if (!session.user) {
        return json(res, 401, {
          error: session.reason === 'account_disabled'
      ...
      user = session.user;
      token = session.token;
      // 슬라이딩 세션: 서버에서 만료가 밀렸으면 브라우저 쿠키 수명도 같이 밀어준다.
      // writeHead 로 Set-Cookie 를 직접 넣는 로그인·로그아웃 라우트는 그쪽 값이 우선한다.
      if (session.renewed) res.setHeader('Set-Cookie', sessionCookie(token));
      ...
      if (user.guest_session_id) {
        ...
        const ended = await resolveGuestClassSession(guestSession);
        if (ended) {
          await destroySession(token);
          return json(res, 401, { error: ended, reason: 'class_ended' });
        }
      }
