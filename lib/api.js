...
// 종량 계측: API 유료 자료 열람만 usage_events에 적재 (무료는 기록 안 함)
async function recordUsage(deck, user, isGuest) {
  if (!deck || deck.cost_type !== 'api_paid') return;
  await q('INSERT INTO usage_events (deck_id, user_id, is_guest) VALUES ($1, $2, $3)',
    [deck.id, user ? user.id : null, !!isGuest]);
}
...
route('GET', /^\/api\/decks\/(\d+)$/, 'student', async (req, res, ctx) => {
  ...
  if (ctx.guestSession) {
    ...
    await recordUsage(deck, ctx.user, true);
  ...
  if (ctx.user.role === 'student') {
    ...
    await log(ctx.user, 'deck_viewed', `id=${deck.id} title=${deck.title}`, req);
    await recordUsage(deck, ctx.user, false);
  }
  const slides = await q('SELECT * FROM slides WHERE deck_id = $1 ORDER BY position, id', [deck.id]);
...
// ================= 종량 과금 리포트 (관리자 이상) =================
// GET /api/billing?month=YYYY-MM → API 유료 자료별 사용 건수·학생 수·예상 비용
route('GET', /^\/api\/billing$/, 'admin', async (req, res, ctx) => {
  const qmonth = new URL(req.url, 'http://internal').searchParams.get('month') || '';
  const month = /^\d{4}-\d{2}$/.test(qmonth) ? qmonth : null;
  // month가 있으면 해당 월만 집계, 없으면 전체 기간 (LEFT JOIN 조건절에서 기간 필터)
  const rows = await q(
    `SELECT d.id, d.title, d.api_provider, d.unit_cost,
       count(ue.id)::int AS uses,
       count(DISTINCT ue.user_id)::int AS learners
     FROM decks d LEFT JOIN usage_events ue ON ue.deck_id = d.id
       ${month ? `AND ue.created_at >= $1::timestamptz AND ue.created_at < ($1::timestamptz + interval '1 month')` : ''}
     WHERE d.cost_type = 'api_paid'
     GROUP BY d.id, d.title, d.api_provider, d.unit_cost
     ORDER BY uses DESC, d.title`,
    month ? [`${month}-01`] : []
  );
  const items = rows.map((r) => ({
    deckId: r.id, title: r.title, apiProvider: r.api_provider, unitCost: Number(r.unit_cost || 0),
    uses: r.uses, learners: r.learners, estCost: Number(r.unit_cost || 0) * r.uses,
  }));
  const total = items.reduce((s, i) => s + i.estCost, 0);
  json(res, 200, { month, items, total });
});
