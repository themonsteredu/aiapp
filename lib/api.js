...
route('POST', /^\/api\/class-sessions$/, 'instructor', async (req, res, ctx) => {
  ...
  // 담당 강사 배정: 관리자 이상만 다른 강사 지정 가능, 강사는 항상 본인
  let instructorId = ctx.user.id;
  if (roleLevel(ctx.user.role) >= roleLevel('admin') && instructor_id) {
    const ins = await one("SELECT id FROM users WHERE id = $1 AND role = 'instructor' AND active = true", [Number(instructor_id)]);
    if (!ins) return badRequest(res, '존재하지 않는 강사입니다.');
    instructorId = ins.id;
  }
  // 6자리 코드 발급 (충돌 시 재시도)
  let cs = null;
