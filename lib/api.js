...
route('POST', /^\/api\/join$/, null, async (req, res, ctx) => {
  ...
  if (!cs || classSessionEndedReason(cs) !== null) {
...
route('POST', /^\/api\/usage\/report$/, null, async (req, res, ctx) => {
  ...
  if (b.token) {
    ...
    // getSessionUser 는 항상 객체를 돌려준다 — 로그인 여부는 user 로 판단해야 한다.
    // (여기서 놓치면 로그인 없이도 사용량을 적재할 수 있어 과금 집계가 오염된다)
    if (!session.user) return cors(401, { ok: false, reason: 'auth' });
    viewer = session.user;
...
// 종료 시각이 지나면 연장·유예 없이 끊는다 (lib/class-session-policy.js 참고).
function classSessionEndedReason(cs) {
  const verdict = classSessionState(cs, Date.now());
  return verdict.action === 'end' ? verdict.message : null;
}
  ...
  for (const r of routes) {
    ...
    if (r.minRole !== null) {
      ...
      if (user.guest_session_id) {
        ...
        const ended = classSessionEndedReason(guestSession);
        if (ended) {
