...
route('POST', /^\/api\/decks$/, 'instructor', async (req, res, ctx) => {
  const { title, description, subject } = ctx.body || {};
  if (!title || !String(title).trim()) return badRequest(res, '제목을 입력하세요.');
  const r = await one(
    'INSERT INTO decks (title, description, subject, created_by) VALUES ($1, $2, $3, $4) RETURNING id',
    [String(title).trim(), String(description || '').trim(), String(subject || '').trim().slice(0, 50), ctx.user.id]
  );
...
route('PATCH', /^\/api\/decks\/(\d+)$/, 'instructor', async (req, res, ctx) => {
  ...
  const { title, description, published, target_classes, access_start, access_end, subject } = ctx.body || {};
