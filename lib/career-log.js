...
function registerCareerLogRoutes({ route, q, one, withTransaction, json, roleLevel, guestDeckAccess, deckVisibleToStudent, checkAccess, todayInTimezone, cookieSecure, schoolStudentId, photoForStudent, photoForStaff }) {
  ...
  route('GET', /^\/api\/career-log\/records$/, 'student', async (req, res, ctx) => {
    ...
    const rows = await q(`SELECT r.id, r.occurred_at, r.process, r.artifact, r.reflection, r.verification_status, r.source, r.program_ref, r.supersedes_id,
        ...
        r.raw_data->'job'->>'student_name' AS student_name, r.raw_data->'job'->'observation' AS observation
    ...
    // 담당자가 넣은 활동 사진. 사진 바이트는 /api/career-photos/<id> 로만 열고 목록에는 이름표만 내려보낸다.
    const photos = page.length
      ? await q('SELECT id, record_id, mime, caption FROM career_record_photos WHERE record_id = ANY($1::uuid[]) ORDER BY position, created_at', [page.map(row => row.id)])
      : [];
    ...
    return send(res, 200, { records: page.map(row => ({ ...row, photos: byRecord.get(row.id) || [] })), hasMore: rows.length > 25 });
  });

  // 활동 사진 한 장. 학생 본인은 자기 번호의 사진만, 담당자는 기록 열람 권한이 있는 학교의 사진만 연다.
  route('GET', /^\/api\/career-photos\/([0-9a-f-]{36})$/i, 'student', async (req, res, ctx) => {
    try {
      let photo;
      if (ctx.user.role === 'student') {
        const who = await identity(req, ctx);
        if (!who) return fail(res, 403, '내 진로기록을 먼저 시작해 주세요.');
        photo = await photoForStudent(who.student_id, ctx.params[0]);
      } else {
        photo = await photoForStaff(ctx.user, ctx.params[0]);
      }
      const buf = Buffer.from(photo.data, 'base64');
      res.writeHead(200, { 'Content-Type': photo.mime, 'Content-Length': buf.length, 'Cache-Control': 'private, no-store', Vary: 'Cookie' });
      return res.end(buf);
    } catch (error) {
      return fail(res, error.status || 500, error.status ? error.message : '사진을 불러오지 못했습니다.');
    }
  });
