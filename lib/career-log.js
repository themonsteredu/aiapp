...
function registerCareerLogRoutes({ route, q, one, withTransaction, json, roleLevel, guestDeckAccess, deckVisibleToStudent, checkAccess, todayInTimezone, cookieSecure }) {
  ...
  route('GET', /^\/api\/career-log\/profile$/, 'student', async (req, res, ctx) => {
    ...
    return send(res, 200, { active: !!active, guest: !!ctx.user.guest_session_id, canResume: !active && !!ctx.user.guest_session_id && !!(await resumeIdentity(req)) });
  });
  ...
  route('GET', /^\/api\/career-log\/records$/, 'student', async (req, res, ctx) => {
    ...
    if (ctx.user.role === 'student') {
      ...
      scope = `${sourceScope} AND r.student_id = $1`;
    ...
    const rows = await q(`SELECT r.id, r.occurred_at, r.process, r.artifact, r.reflection, r.verification_status,
        r.raw_data->'job'->>'deck_title' AS title, r.raw_data->'job'->>'session_title' AS session_title,
