...
-- 초기 시범 운영 때 만들어진 테이블도 현재 상세 화면에서 안전하게 읽을 수 있도록
-- 누락 가능성이 있는 열만 보완한다. ADD COLUMN IF NOT EXISTS라 기존 데이터는 유지된다.
ALTER TABLE project_teams ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT '미시작';
ALTER TABLE project_teams ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT now();
ALTER TABLE project_teams ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ NOT NULL DEFAULT now();

ALTER TABLE project_members ADD COLUMN IF NOT EXISTS team_role TEXT NOT NULL DEFAULT '';
ALTER TABLE project_members ADD COLUMN IF NOT EXISTS last_seen_at TIMESTAMPTZ NOT NULL DEFAULT now();
ALTER TABLE project_members ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT now();

ALTER TABLE session_progress ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT '미시작';
ALTER TABLE session_progress ADD COLUMN IF NOT EXISTS draft_data JSONB NOT NULL DEFAULT '{}'::jsonb;
ALTER TABLE session_progress ADD COLUMN IF NOT EXISTS confirmed_data JSONB;
ALTER TABLE session_progress ADD COLUMN IF NOT EXISTS last_saved_at TIMESTAMPTZ;
ALTER TABLE session_progress ADD COLUMN IF NOT EXISTS confirmed_at TIMESTAMPTZ;

ALTER TABLE published_apps ADD COLUMN IF NOT EXISTS active BOOLEAN NOT NULL DEFAULT true;
ALTER TABLE published_apps ADD COLUMN IF NOT EXISTS published_at TIMESTAMPTZ NOT NULL DEFAULT now();
...
// exec = 원문 SQL 실행 함수, q = 파라미터 쿼리 함수.
// 초기화는 자문 잠금을 쥔 단일 커넥션 위에서 돌아가므로, 풀이 아니라 그 커넥션의 실행기를 받는다.
async function initProjectSchema(exec, q) {
  await exec(PROJECT_SCHEMA);
