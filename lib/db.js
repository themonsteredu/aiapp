...
const { initProjectSchema } = require('./project-schema');
...
async function withTransaction(work) {
  ...
  const txQ = async (sql, params = []) => (await client.query(sql, params)).rows;
  const txOne = async (sql, params = []) => (await txQ(sql, params))[0] || null;
  try {
    await client.query('BEGIN');
    const result = await work({ q: txQ, one: txOne });
...
-- 세션은 요청마다 조회되고, user_id 로 훑는 쿼리가 여러 곳이다 (동시접속 제한·계정 비활성·수업 종료)
CREATE INDEX IF NOT EXISTS idx_sessions_user ON sessions(user_id);
CREATE INDEX IF NOT EXISTS idx_sessions_expires ON sessions(expires_at);
-- assets 는 base64 blob 을 같은 행에 담고 있어 순차 스캔 비용이 특히 크다
CREATE INDEX IF NOT EXISTS idx_assets_deck ON assets(deck_id);
`;
...
// 스키마·시드 버전. SCHEMA / MIGRATIONS / 시드를 건드리면 이 값을 올려야 반영된다.
// 값이 DB에 기록된 것과 같으면 콜드스타트에서 DDL·시드를 통째로 건너뛴다 (왕복 1회로 끝).
const SCHEMA_VERSION = 2;
// 여러 인스턴스가 동시에 초기화를 시작해도 한 번만 돌게 하는 자문 잠금 키 (임의 상수)
const INIT_LOCK_KEY = 726143001;
...
// 이미 현재 버전으로 초기화된 DB인가 (왕복 1회). 테이블 자체가 없으면 최초 실행.
async function schemaIsCurrent(exec) {
  try {
    const r = await exec("SELECT value FROM schema_meta WHERE key = 'version'");
    return Number(r.rows[0]?.value) >= SCHEMA_VERSION;
  } catch {
    return false;
  }
}

// 실제 스키마·시드 적용. exec = 원문 SQL 실행, cq = 파라미터 쿼리 (둘 다 같은 커넥션)
async function applySchema(exec, cq, one1) {
  await exec(SCHEMA_META_TABLE);
  await exec(SCHEMA);
  await exec(MIGRATIONS);
  await exec(AGREEMENTS_TABLE);
  await exec(COURSES_TABLE);
  await exec(USAGE_TABLE);
  await exec(SETTLEMENTS_TABLE);
  await initProjectSchema(exec, cq);

  // 슈퍼관리자 시드
  const su = await one1("SELECT count(*)::int AS c FROM users WHERE role = 'superadmin'");
  if (su.c === 0) {
  ...
  const sc = await one1('SELECT count(*)::int AS c FROM schedules');
  if (sc.c === 0) {
    for (const dow of [1, 2, 3, 4, 5]) {
      await cq("INSERT INTO schedules (day_of_week, start_time, end_time) VALUES ($1, '09:00', '18:00')", [dow]);
    }
  }
  // 보안 설정 기본값
  const defaults = {
  ...
  for (const [k, v] of Object.entries(defaults)) {
    await cq('INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO NOTHING', [k, v]);
  }
  // 외부 웹앱 게이트 토큰 서명용 비밀키 (최초 1회 생성, 클라이언트에 노출 금지)
  await cq('INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO NOTHING',
...
// 서버리스 콜드스타트마다 DDL 수십 건을 실행하면 첫 요청이 몇 초씩 느려진다.
// 이미 현재 버전이면 확인 1회로 끝내고, 실제로 적용해야 할 때만 잠금을 잡고 한 번만 돌린다.
async function init() {
  if (await schemaIsCurrent((sql) => pool.query(sql))) return;

  const client = await pool.connect();
  // 잠금을 잡은 뒤에는 이 커넥션으로만 질의한다.
  // (Vercel 은 풀 크기가 1이라, 잠금을 쥔 채 pool.query 를 부르면 자기 자신을 기다리다 멈춘다)
  ...
  try {
    ...
    // 기다리는 사이 다른 인스턴스가 끝냈을 수 있다 — 다시 확인한다
    if (await schemaIsCurrent(exec)) return;
    await applySchema(exec, cq, cOne);
  } finally {
...
// 설정은 요청마다 여러 번(인증 게이트 + 각 라우트) 조회되는데 거의 바뀌지 않는다.
// 인스턴스 안에서 잠깐 캐시해 왕복을 줄인다. 같은 인스턴스의 변경은 즉시 반영되고,
// 다른 인스턴스에는 최대 TTL 만큼 늦게 퍼진다.
...
async function setSetting(key, value) {
  ...
  invalidateSettings(); // 저장 직후 화면에 옛 값이 보이지 않도록
}
...
module.exports = { q, one, withTransaction, ready, log, getSettings, setSetting, clientIp, TS, KST_TODAY, DEFAULT_AGREEMENT };
