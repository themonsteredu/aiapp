...
CREATE TABLE IF NOT EXISTS users (
  ...
  name TEXT NOT NULL,
  role TEXT NOT NULL CHECK (role IN ('superadmin','admin','instructor','partner','student')),
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS school_account_id UUID UNIQUE;              -- 모아허브 학교 학생 계정(moakit_accounts.accounts.id) 연결
...
-- 진로업체 담당자(partner) 역할 추가. 기존 DB의 CHECK 제약을 다시 만든다.
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_role_check;
ALTER TABLE users ADD CONSTRAINT users_role_check CHECK (role IN ('superadmin','admin','instructor','partner','student'));
-- 진로 관찰 기록에 붙는 활동 사진. 기록 원본(career_log.records)은 append-only 라 사진은 이 표에 따로 둔다.
-- 열람 권한은 record_id 가 아니라 student_uuid·school_id 로 확인한다 (기록이 정정돼도 사진 주인은 그대로).
CREATE TABLE IF NOT EXISTS career_record_photos (
...
CREATE INDEX IF NOT EXISTS career_record_photos_record_idx ON career_record_photos (record_id, position);
CREATE INDEX IF NOT EXISTS career_record_photos_student_idx ON career_record_photos (student_uuid);
`;
...
async function init() {
  ...
  await initProjectSchema(pool, q);
