...
function registerSchoolRegistryRoutes({ route, one, json, log, registry }) {
  ...
  // minRole 이 'partner' 인 것은 진로업체 담당자 계정을 위해서다. 실제 범위는 registry 의 record_access 가 정한다.
  route('GET', /^\/api\/school-accounts\/record-schools$/, 'partner', guard(async (req, res, ctx) =>
    send(res, 200, { schools: await registry.recordSchools(ctx.user) })));

  route('GET', path('/record-students'), 'partner', guard(async (req, res, ctx) =>
    send(res, 200, await registry.members(ctx.user, ctx.params[0]))));

  route('GET', path(`/students/${ID}/records`), 'partner', guard(async (req, res, ctx) => {
  ...
  route('GET', path(`/students/${ID}/records/${ID}/history`), 'partner', guard(async (req, res, ctx) =>
    send(res, 200, await registry.recordHistory(ctx.user, ctx.params[0], ctx.params[1], ctx.params[2]))));

  route('POST', path(`/students/${ID}/records`), 'partner', guard(async (req, res, ctx) => {
  ...
  route('POST', path(`/students/${ID}/records/${ID}/revise`), 'partner', guard(async (req, res, ctx) => {
  ...
  route('PUT', path('/record-access/(\\d{1,18})'), 'admin', guard(async (req, res, ctx) => {
    ...
    if (!target || !target.active || target.school_account_id || !['partner', 'instructor', 'admin', 'superadmin'].includes(target.role)) {
      return send(res, 400, { error: '활성 강사·관리자·진로업체 담당자 계정을 확인하세요.' });
    }
